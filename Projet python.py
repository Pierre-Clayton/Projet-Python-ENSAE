...
def countries_to_iso(country):
    name_countries = iso['Country']
    for i in range(len(name_countries)):
        if name_countries[i] == country:
            return iso.loc[i,'Alpha-3 code']
    list_missing_values = ['United Kingdom of Great Britain and Northern Ireland',
     'Russian Federation',
     'United States of America',
     'Netherlands',
...
#%%
#number of the five most common coding language


def number_of_most_spoken(haveworkedwith):
    if type(haveworkedwith) == str:
        most_common_language = ["JavaScript","Python","HTML/CSS","SQL","Java"]
        list_language = haveworkedwith.split(";")
        number = 0
        for language in list_language:
            if language in most_common_language:
                number += 1
        return number
               
df["HaveWorkedWith5MostCommonLanguage"] = df["HaveWorkedWith"].apply(number_of_most_spoken)
#%%
 # Finding the number of None values in the column HaveWorkedWith5MostCommonLanguage
none_count_hww5mcl = df['HaveWorkedWith5MostCommonLanguage'].isnull().sum()

 # Extracting the indices of None values
none_indices_hww5mcl = df.index[df['HaveWorkedWith5MostCommonLanguage'].isnull()].tolist()

 #Extracting the indices of missing values
values_at_indices_hww5mcl = df['HaveWorkedWith'].iloc[none_indices_hww5mcl]

 #List of all missing values
data_without_duplicates_hww5mcl = values_at_indices_hww5mcl.drop_duplicates()